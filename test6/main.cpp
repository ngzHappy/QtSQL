...
void create_sqlite_table() {
    ...
    {
        QSqlQuery varSQLQuery{ varDataBase };
        if(varSQLQuery.prepare(QStringLiteral(R"^s.q.l^(INSERT INTO student VALUES(:id,:name,:age))^s.q.l^"))) {
            qWarning() << QStringLiteral("OK prepare") << __func__;
        } else {
            qWarning() << varDataBase.lastError();
        }
        {
            varSQLQuery.bindValue(QStringLiteral(":id"),1 );
            varSQLQuery.bindValue(QStringLiteral(":name"),QStringLiteral("Zhao"));
            varSQLQuery.bindValue(QStringLiteral(":age"),10);
            varSQLQuery.exec();
        }
        {
            varSQLQuery.bindValue(QStringLiteral(":id"),2);
            varSQLQuery.bindValue(QStringLiteral(":name"),QStringLiteral("Qian"));
            varSQLQuery.bindValue(QStringLiteral(":age"),20);
            varSQLQuery.exec();
        }
        {
            varSQLQuery.bindValue(QStringLiteral(":id"),3);
            varSQLQuery.bindValue(QStringLiteral(":name"),QStringLiteral("Sun"));
            varSQLQuery.bindValue(QStringLiteral(":age"),30);
            varSQLQuery.exec();
        }
    }
